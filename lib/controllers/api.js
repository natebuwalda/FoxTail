...
exports.updateWebsite = function (req, res) {
  ...
  myProcess.on('close', function (code) {
    req.io.sockets.emit('status', {mech: 'updating', set: false, code: code});
  });

  req.io.sockets.emit('status', {mech: 'updating', set: true});
};
...
exports.buildWebsite = function (req, res) {
  ...
  myProcess.on('close', function (code) {
    req.io.sockets.emit('status', {mech: 'rebuilding', set: false, code: code});
  });

  req.io.sockets.emit('status', {mech: 'rebuilding', set: true});
};
...
exports.stopWebsite = function (req, res) {
  res.send(200);
  var myProcess = spawn('./grunt', ['kira'], {cwd: getWd(req.mode)});
  ...
  myProcess.on('close', function (code) {
    req.io.sockets.emit('status', {mech: 'restarting', set: false});
  });

  req.io.sockets.emit('status', {mech: 'restarting', set: true});
};
